...
| `archive_emails` | Archive multiple emails at once via `batchModify` (1-1000 ids — Gmail's own limit, one confirmation) |
| `delete_email` | Move an email to Trash (recoverable for 30 days) |
| `delete_emails` | Move multiple emails to Trash at once via `batchModify` (1-1000 ids — Gmail's own limit, one confirmation) |
